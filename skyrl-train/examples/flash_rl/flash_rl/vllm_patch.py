...
def vllm_model_finder(vllm_llm):
    print("Finding model in vllm_model_finder")
...
def patch_vllm_process_weights_after_loading():
    try:
        ...
        if not hasattr(utils, "beforeflashrl_process_weights_after_loading"):
            ...
            print("adding hacked process weights after loading")
            ...
            def hacked_process_weights_after_loading(
                ...
                if not hasattr(model, "hacked_original_weights_rebuild_keys"):
                    ...
                    for name, p in original_weights.items():
                        model.hacked_original_weights_rebuild_keys[name] = (
                            p.shape,
                            p.stride(),
                            p.dtype,
        ...
        if not hasattr(BaseKVCacheMethod, "beforeflashrl_process_weights_after_loading"):
            ...
            BaseKVCacheMethod.beforeflashrl_process_weights_after_loading = (
                original_kvcache_process_weights_after_loading
            )
...
def patch_vllm_lmhead_to_fp32():
    try:
        print("inside the patching function for lmhead to fp32 now")
        if not hasattr(
            vllm.model_executor.layers.vocab_parallel_embedding.VocabParallelEmbedding,
            "beforeflashrl_VocabParallelEmbedding",
        ):
            ...
            vllm.model_executor.layers.vocab_parallel_embedding.VocabParallelEmbedding.beforeflashrl_VocabParallelEmbedding = (
                original_init
            )

            def hacked_init_(self, *args, **kwargs) -> None:
    ...
    try:
        if not hasattr(
            vllm.model_executor.layers.vocab_parallel_embedding.UnquantizedEmbeddingMethod, "beforeflashrl_apply"
        ):
            # Store the original LLM init function
            ...
            def hacked_apply_(self, layer: torch.nn.Module, x: torch.Tensor, bias=None) -> torch.Tensor:
    ...
    try:
        ...
        if not hasattr(Sampler, "beforeflashrl_forward"):
            ...
            def hacked_logprob_forward(
                ...
                ) or (isinstance(sampling_metadata.temperature, float) and sampling_metadata.temperature != 1.0):
...
def load_flashrl_config(config):
    ...
    if config_path in ["bf16", "fp8", "fp8_vllm"]:
        ...
        from examples.flash_rl.flash_rl.configs import get_default_config
...
def patch_vllm_llm():
    try:
        # patch this again here
        patch_vllm_process_weights_after_loading()
        print("inside the patching function now")
        if not hasattr(vllm.LLM, "beforeflashrl__init__"):
            # Store the original LLM init function
            print("further inside the patching function now")
            original_init = vllm.LLM.__init__
            vllm.LLM.beforeflashrl__init__ = original_init

            def hacked_init_(self, model: str, **kwargs) -> None:
                print("inside hacked init")
                ...
                if config is not None:
                    ...
                    # hardocded to 0, we only manage at engine level in skyrl, not rank level
                    config_index = 0
                    ...
                    if config_data.get("fn", "int8") != "bf16":
                        ...
                        if config_data.get("fn", "int8") == "fp8_vllm":
                            if "profile" in config_data:
                                logger.warning(
                                    f"flash_rl fp8_vllm profile is not needed, but set as {config_data['profile']}"
                                )
                            ...
                            if not os.path.exists(quant_profile_path):
                                ...
                                assert (
                                    len(quant_profile_path) >= 3
                                ), f"Invalid flash_rl profile path: {quant_profile_path}"
                                quant_profile_path = hf_hub_download(
                                    repo_id="/".join(quant_profile_path[:2]), filename="/".join(quant_profile_path[2:])
                                )
                    ...
                    if "module_attribute_to_preserve" in config_data:
                        logger.debug(
                            f"flash_rl module_attribute_to_preserve: {config_data['module_attribute_to_preserve']}"
                        )
...
def patch_vllm_llm_test_reload():
    try:
        if not hasattr(vllm.LLM, "beforeflashrl_test__init__"):
            ...
            def test_reload_at_init_(self, *args, **kwargs) -> None:
                # Call the parent's __init__ with the custom model
                ...
                if config is not None:
                    ...
                    for config_i in config:
                        model_to_be_reloaded = AutoModelForCausalLM.from_pretrained(
                            config_i, device_map="cpu", torch_dtype="auto"
                        )
